...
@torch.no_grad()
def get_query_key_matrix(model, tokenizer, args, dev):
    ...
    cache_file = f"cache/ours/{model_id.replace('/','_')}_calibration_matrices_fp16.pt"
    os.makedirs("cache/ours", exist_ok=True)

    if os.path.exists(cache_file) and args.use_cache:
        logger.info(f"[ours] File {cache_file} exist.", fg="green")
        logger.info(f"[ours] Load calibration matrix from cache: {cache_file}", fg="yellow")
        calibration_matrices = torch.load(cache_file, map_location="cpu")
        ...
        for i in tqdm(range(len(layers))):
            ...
            for name in subset:
                if "k_proj" in name:
                    query_subspace = calibration_matrices[i][0]
                    subset[name].query_subspace = query_subspace
                    key_matrix = calibration_matrices[i][1]
                    subset[name].matrix = key_matrix

        return 

    logger.info(f"No cache_file={cache_file}", fg="red")
    logger.info(f"Create query key matrix dict...", fg="yellow")
    ...
    logger.info("[Decomposition] Start to calculate the q, k matrix in layer-wise manner...")
    for i in tqdm(range(len(layers))):
        ...
        def hook_q(module, input, output):
            ...
            if module.matrix == None:
                module.matrix = q_matrix.detach().clone()
            else:
        ...
        for name in subset:
            if "k_proj" in name:
                ...
                subset[name].query_subspace = query_subspace
                torch.cuda.empty_cache()
        calibration_matrices.append((query_subspace, key_matrix))
        layers[i] = layer.cpu()
        inps = outs
        torch.cuda.empty_cache()
...
def compress_model_ours(model, tokenizer, args, dev, selection_result):
    logger.info("Compressing model with orthogonality...")
    # NOTE(brian1009): Prepare whiten scaling matrix
    get_query_key_matrix(model, tokenizer, args, dev)
    # Compress the model
    module_dict = {name: module for name, module in model.named_modules()}
    full_name_dict = {module: name for name, module in model.named_modules()}
    linear_info = {}
