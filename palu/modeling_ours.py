...
class LlamaAttention_ours(nn.Module):
    ...
    def get_query_subspace(self, query_states, subspace_dim):
        U, _, Vh = torch.linalg.svd(query_states.float(), full_matrices=False)
        Vh_subspace = Vh[:subspace_dim, :].to(query_states.dtype)
        return Vh_subspace
...
def replace_llama_attention_modules(module, config):
    AttentionClasses = tuple(LLAMA_ATTENTION_CLASSES.values())
    for name, child in module.named_children():
        if isinstance(child, AttentionClasses):
            print(f"[AttentionReplace] {module.__class__.__name__}.{name} (layer_idx={child.layer_idx})")
            new_attn = LlamaAttention_ours(child.config, child.layer_idx)
            new_attn.load_state_dict(child.state_dict(), strict=False)
            setattr(module, name, new_attn)
        else:
            replace_llama_attention_modules(child, config)
